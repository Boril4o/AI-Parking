...
class ParkingEnvironment(gym.Env):
    """
    A simple 2D parking environtment.

    Playground with four walls. Four parking spots at the top.
    With random picked parking spot and random picked taken spots by cars.
    """
    ...
    playground_width = parking_spot_width * parking_spot_count_on_side + wall_width * 2
    playground_height = (parking_spot_height * 2) + (wall_width * 2) + 5
    parked_cars_count = 3
    raycast_length = 100.0
    car_default_center = (playground_width / 2, playground_height / 2 + car_height * 2)
    turn_speed = 50.0
    car_speed = 50.0
    dt = 1 / 60
    max_step = 2000
    ...
    parked_reward = 50
    closer_to_park_spot_reward = 0.05
    ...
    def __init__(self, render_mode=None):
        ...
        car_angle: current car angle
        velocity: current velocity of the car
        raycasts: raycasts four of which start at each corner of the car, two in the front and the back side of the car and two on the left and ride side
        """
        ...
        #Two actions first for velocity and second for angular velocity
        ...
        # Calculate the furthest possible distance between the car and a parking spot
        # Used later to normalize distances to [0, 1]
        ...
        # Take the larger of the two to cover both horizontal and vertical parking spots
        ...
        #surface for obstacles only, used for colision
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # Track the closest the car has ever been to the target spot this episode (normalized)
        ...
        # Create the car surface and place it at the default starting position
        ...
        # Rotate the car surface to match the starting angle for rendering
        ...
        for parking_spot in self.parking_spots:
            ...
            # Fill spots randomly until we reach the max parked cars count
            if len(parked_cars) < self.parked_cars_count and self.np_random.uniform(0.0, 1.0) < 0.5:
                # Parked cars are rotated 90 degrees so width and height are swapped
        ...
        # Reset to just the four walls first, then add this episode's parked cars
        ...
        # Clear the obstacles surface so it only has this episode's obstacles
        self.obstacles_surf.fill((0, 0, 0, 0))

        # Draw walls on the background and on the obstacles surface
    ...
    def render(self):
        ...
        if not self.screen:
            if self.render_mode == "human":
                ...
                # Off-screen surface used when we just need pixel output without a window
        ...
        # Draw obstacle rays, shortening them to the hit point if there was one
    ...
    def get_raycast(self):
        ...
        # Midpoints of each of the four sides in local space
        sides_offsets = [
        ...
        # Return two lists: one for obstacle hits and one for parking-spot hits
        return [
            [
    ...
    def get_obs(self):
        ...
        #we find min accessable car position by adding 0(start) + car width/height / 2
        min_car_x = self.car_width_pixels / 2
        min_car_y = self.car_height_pixels / 2

        #we find max accesable car position by subtracting car width/heigh / 2 from playground size
        max_car_x = self.playground_width_pixels - self.car_width_pixels / 2
        max_car_y = self.playground_height_pixels - self.car_height_pixels / 2

        #normalazing cars position based on the min/max accessable positions
        car_x_pos = (self.car_pos[0] - min_car_x) / (max_car_x - min_car_x)
        car_y_pos = (self.car_pos[1] - min_car_y) / (max_car_y - min_car_y)

        # Clip the values so they can never, ever go outside 0.0 and 1.0
        car_x_pos = np.clip(car_x_pos, 0.0, 1.0)
        car_y_pos = np.clip(car_y_pos, 0.0, 1.0)

        #print(f"car pos: {self.car_pos}")
        #print(f"velocity: {car_velocity}")
        #print(f"angle: {angle}")
        #print(f"raycasts: {raycasts}")
        #print(f"dx: {dx_normalized}, dy: {dy_normalized}")
        #print(f"dx: {car_x_pos}, dy: {car_y_pos}")
        #print("-------------------------------------------")
        ...
        return {
            ...
            "position": np.array([car_x_pos, car_y_pos], dtype=np.float32),
